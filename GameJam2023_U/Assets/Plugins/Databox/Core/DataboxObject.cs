...
namespace Databox
{
	...
	public class DataboxObject : ScriptableObject {
		...
		{
			if (serializer == Serializer.FullSerializer)
			{
				//full serializer
				fsSerializer _serializer = new fsSerializer();
				fsData data;
			...
			{
				#if NET_4_6
				var _bytes = SerializationUtility.SerializeValue(_db, DataFormat.JSON);
				var _jsonString = System.Text.Encoding.UTF8.GetString(_bytes);
				return _jsonString;
				#else
				Debug.LogWarning("Please change API .NET to 4.X");
				return "";
				#endif
			}
		}
		...
		public OrderedDictionary<string, Database> JsonToDB(string _json, string _version, bool _isNewVersion)
		{
			...
			if (serializer == Serializer.FullSerializer)
			{
				// Some special characters cant be serialized with fullserializer
				// It is recommended to use odin serializer.
				try
				{
					fsSerializer _serializer = new fsSerializer();
				...
				{
					//Debug.LogWarning("Exception Error while parsing! Please try Odin Serializer");
					cloudStatus += "- Exception Error while parsing! There might be unsupported characters! Please try Odin Serializer" + "\n";
					cloudWarnings = CloudWarnings.warning;
					return null;
				}
			}
			else
			{
				#if NET_4_6
				var _bytes = System.Text.Encoding.UTF8.GetBytes(_json);
				var _data = SerializationUtility.DeserializeValue<OrderedDictionary<string, Database>>(_bytes, DataFormat.JSON);
